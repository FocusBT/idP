#!/usr/bin/env node
/*
 * Benchmark script for NestJS zk‑authentication prototype
 * -------------------------------------------------------
 * Re‑written to use the Autocannon “requests” array so that each
 * endpoint’s path is honoured.  This removes the 404/“0 2xx” issue.
 */
...
(async function main () {
  ...
  // --- pre‑compute secret / proof / signals so they’re valid for all tests ----
  let secretHex, commitment, proof, publicSignals;
  try {
    const reg = await axios.post(`${BASE_URL}/auth`, buildUser());
    secretHex  = reg.data.secret;
    commitment = reg.data.commitment;

    const prf = await axios.post(`${BASE_URL}/auth/proof`, { secretHex, commitment });
    proof         = prf.data.proof;
    publicSignals = prf.data.publicSignals;

    await axios.post(`${BASE_URL}/auth/verify`, { commitment, proof, publicSignals });
  } catch (e) {
  ...
  const endpoints = [
    ...
    },
    {
      ...
      requests : [{
        ...
        path   : '/auth/proof',
        headers: { 'Content-Type': 'application/json' },
        body   : JSON.stringify({ secretHex, commitment })
      }]
    },
    {
      name     : 'verifyProof',
